	...
	<body>
		...
		<div id="show-pais" class="content scaffold-show" role="main">
			...
			<ol class="property-list pais">
				...
				<li class="fieldcontain">
					<span id="provincia-label" class="property-label"><g:message code="pais.provincia.label" default="Provincia" /></span>
					
						<g:each in="${paisInstance.provincia}" var="p">
						<span class="property-value" aria-labelledby="provincia-label"><g:link controller="provincia" action="show" id="${p.id}">${p?.encodeAsHTML()}</g:link></span>
						</g:each>
